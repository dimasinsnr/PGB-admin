<div class="custom-dropdown" id="customDropdown">
    ...
    <div class="custom-dropdown-menu" aria-labelledby="customDropdown" onclick="closeCustomDropdowns(event)">
        <a class="dropdown-item" href="javascript:void(0)" onclick="onEdit('{{ strval($anggota_id) }}', '{{ strval($anggota_nama) }}', 'detailAnggota')">
            <i class="fas fa-user fa-sm fa-fw mr-2 text-gray-400"></i>
            {{ __('Detail') }}
        </a>
        <a class="dropdown-item" href="javascript:void(0)" onclick="onEdit('{{ strval($anggota_id) }}', '{{ strval($anggota_nama) }}', 'formAnggota')">
            <i class="fas fa-user fa-sm fa-fw mr-2 text-gray-400"></i>
            {{ __('Edit') }}
        </a>
        <a class="dropdown-item" href="javascript:void(0)" onclick="onDelete('{{ strval($anggota_id) }}')">
    ...
    <ul class="navbar-nav">
        <li class="nav-item dropdown no-arrow">
            ...
            <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in static" style="z-index: 2" aria-labelledby="opDD">
                <a class="dropdown-item" href="javascript:void(0)" onclick="onEdit('{{ strval($anggota_id) }}', '{{ strval($anggota_nama) }}')">
                    <i class="fas fa-user fa-sm fa-fw mr-2 text-gray-400"></i>
                    {{ __('Edit') }}
                </a>
                <a class="dropdown-item" href="javascript:void(0)" onclick="onDelete('{{ strval($anggota_id) }}')">
